...
import os
import psutil  # Replacing netifaces with psutil
...
class DNSResolver:
    def __init__(self, dns_server='8.8.8.8', timeout=2, retries=3):
        ...
        try:
            ...
            for iface in non_loopback_interfaces:
                ...
                inet_addr = next((addr for addr in addrs if addr.family == socket.AF_INET), None)
                link_addr = next((addr for addr in addrs if addr.family == psutil.AF_LINK), None)

                if inet_addr and link_addr:
                    self.interface = iface
                    self.src_mac = self.mac_to_bytes(link_addr.address)
                    self.src_ip = socket.inet_aton(inet_addr.address)
                    self.log(f"Using physical interface {iface} with IP {inet_addr.address}")
                    break

            if not self.interface:
                self.log("No suitable interface found, raw sockets disabled.")
                return

            # Get gateway MAC address for routing
            try:
                # Find the gateway IP associated with the chosen interface
                gateway_ip = None
                gateways = psutil.net_if_gateways()
                if gateways.get(socket.AF_INET):
                    for gw_iface, gw_addr, _ in gateways[socket.AF_INET]:
                        if gw_iface == self.interface:
                            gateway_ip = gw_addr
                            self.log(f"Found default gateway: {gateway_ip} on interface {self.interface}")
                            break

                if gateway_ip:
                    # Use ARP to get gateway MAC
                    self.dst_mac = self.get_mac_address(gateway_ip)
                    if not self.dst_mac:
                        self.log("Could not determine gateway MAC address via ARP")
                        self.dst_mac = b'\xff\xff\xff\xff\xff\xff' # Fallback to broadcast
                        self.log("Using broadcast MAC address")
                else:
                    self.log("No default gateway found for the interface")
                    self.dst_mac = b'\xff\xff\xff\xff\xff\xff' # Fallback to broadcast
                    self.log("Using broadcast MAC address")
            ...
            # Create a second socket to receive responses
        ...
        # Catch errors if psutil is missing or if raw sockets fail (e.g., on Windows)
        except (ImportError, OSError, AttributeError) as e:
            self.log(f"Raw socket creation failed (this is expected on Windows): {e}")
    ...
    def mac_to_bytes(self, mac_str):
        """Convert MAC address string to bytes"""
         # Remove both colons (Linux/macOS) and hyphens (Windows)
    ...
    def send_query(self, domain):
        # --- MODIFICATION ---
        # The domain restriction block has been removed from here.
        # --------------------
